...
c_tcp_async_linker::c_tcp_async_linker()
{
    m_fd = -1;
    m_recv_pos = 0;
    m_recv_buf = NULL;
...
bool c_tcp_async_linker::start(int fd, sockaddr_in &ip)
{
    m_fd = fd;
    m_ip = ip;
    ...
    while (true) {
        int ret = connect(m_fd, (struct sockaddr *)&ip, sizeof(ip));
    ...
    // 触发EPOLLOUT即连接成功
    if (!g_reactor.handle_ctl(m_fd, EPOLL_CTL_ADD, EPOLLOUT)) {
        handle_fini();
        return false;
    }
...
bool c_tcp_async_linker::handle_input()
{
    if (!m_is_connected) {
        // TODO
        return false;
...
READ:
    len = g_plugin.get_pkg_len_ser(m_fd, (char*)m_recv_buf + pos, m_recv_pos - pos);
...
bool c_tcp_async_linker::handle_output()
{
    if (!m_is_connected) {
        // 建立连接成功
        ...
        if (g_plugin.link_up_ser)
            g_plugin.link_up_ser(m_fd, m_ip.sin_addr.s_addr, m_ip.sin_port);
...
void c_tcp_async_linker::handle_error()
{
    if (!m_is_closed) {
...
void c_tcp_async_linker::handle_fini()
{
    if (m_fd != -1) {
...
void c_tcp_async_linker::send_pkg(const void * buf, uint32_t len)
{
    if (!m_send_list.empty()) {
        uint32_t p = 0;
