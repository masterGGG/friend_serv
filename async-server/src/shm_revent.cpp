...
bool c_revent::handle_input()
{
    ...
    while (count > 0) {
        ...
        if (sb.type == PROTO_BLOCK) {
			...
			DEBUG_LOG("[2]Get msg from connect process: RECEIVE BUFFER:[%s]BUFFER LENGTH:[%d]", logbuf, sb.len);
			DEBUG_LOG("[2]Get msg from connect process: cli_fd[%d], sb_id[%d], channle[%d]", sb.fd, sb.id, m_channel);
