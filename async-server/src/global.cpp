...
char g_proc_name[32] = {0};
int  g_work_num = 0;

std::vector<work_conf_t> g_work_confs;

int load_proc_name(const char *proc_name)
{
    if (!proc_name) {
        BOOT_LOG(-1, "load null proc name");
    }
...
int load_work_num(int work_num)
{
    if (work_num <= 0) {
        BOOT_LOG(-1, "load work num:%d", work_num);
    }
...
extern "C" const char *get_proc_name()
{
    return g_proc_name;
}
...
{
    if(g_plugin.config_get_strval("warning_server_ip", "0.0.0.0") == 0)
    {
        return;
    }

    warning_fd = create_udp_socket(&warning_sockaddr, g_plugin.config_get_strval("warning_server_ip", "0.0.0.0"), g_plugin.config_get_intval("warning_port", 0));
    if(warning_fd == -1)
    {
        WARN_LOG("failed to init warning sys: ip=%s, port=%d", g_plugin.config_get_strval("warning_server_ip", "0.0.0.0"), g_plugin.config_get_intval("warning_port", 0));
    }

    prj_name = g_plugin.config_get_strval("project_name", 0);
    if(prj_name == 0)
    {
        prj_name = "unspecified";
    }

    phone_nums = g_plugin.config_get_strval("phone_number", 0);
    if(phone_nums == 0)
    {
