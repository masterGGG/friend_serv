    ...
    if (len == 0) {
        ...
        if (pos == 0 && (uint32_t)len > m_recv_buf_size) { // realloc recvbuf
            ...
            if (channel == -1) {
				...
				ASC2HEX_TEST(logbuf, (char *)(m_recv_buf + m_recv_pos), (len > PACKAGE_MAX_SIZE) ? PACKAGE_MAX_SIZE : len);
				DEBUG_LOG("[1]Connect process get msg from client: RECEIVE BUFFER:[%s]BUFFER LENGTH:[%d]", logbuf, len);
				DEBUG_LOG("[1]Connect process get msg from client: cli_fd[%d], sb_id[%d], channel[%d]", m_fd, m_id, channel);
                shm_block_t sb;
                sb.fd = m_fd;
                sb.id = m_id;
                sb.len = len;
