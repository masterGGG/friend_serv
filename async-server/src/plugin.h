...
struct plugin_t {
    ...
    bool (*load_config)(const char *source, int type);
    bool (*reload_config)(const char *source, int type);
     ...
     * @param len
     */
    int (*get_pkg_len_ser)(int fd, const char *buf, uint32_t len);
    ...
    */
    ...
    void (*link_up_ser)(int fd, uint32_t ip, uint16_t port);
